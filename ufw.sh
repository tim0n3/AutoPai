...
_ddos_firewall_rules() {
	...
	echo -e "# Mangle Rules:\n"
	iptables -t mangle -A PREROUTING -m conntrack --ctstate INVALID -j REJECT --reject-with icmp-proto-unreachable
	...
	iptables -t mangle -A INPUT -m conntrack --ctstate INVALID -j REJECT --reject-with icmp-proto-unreachable
